...
func NewStageDeps(agent agents.StageAgent, h *home.Home, cfg config.Config, provider forge.Provider,
	redactor redact.Redactor, git ...vcs.Option) StageDeps {
	return StageDeps{Agent: agent, Home: h, Config: cfg, Forge: provider, redact: redactor, git: git}
}
