...
// lintProjectionBytes bounds the command output that travels in the stage's
// report. The whole output goes to the run's lint evidence file, and what a
...
const lintProjectionBytes = 8 << 10

// Lint is the lint stage: it runs static analysis over the change.
//
// # What it establishes, and what it does not
//
// PRD section 5 asks this stage for static analysis of this change, and PRD
// section 10 puts the check itself in commands.lint. So the check lives in the
// configured command, and this stage runs that command rather than deriving
// one: a stage that appended paths to a command line it cannot parse would be
// inventing a second targeting rule, quietly, out of a value whose whole shape
...
// allow_pushed_commands: commands.lint is config.TrustCommands rather than
...
// no check can establish validity - write one, verify manually with recorded
// evidence, or report that sufficient evidence is not possible - and this
// build always takes the third. The first two need an agent authoring a check
// and a person recording a verification, and neither is built; a stage that
// took either silently would be reporting a pass it did not establish.
//
// # Evidence
//
// PRD section 8 puts lint evidence at evidence/<run>, deliberately outside the
// isolated copy, so that a lint artifact never becomes part of the change
...
//
// That file is the run's lint evidence and nothing more. PRD section 8's
...
// whether a check establishes validity would be making such a claim and would
// owe that rule; it would not inherit an exemption from here.
//
...
// The key is documented as static analysis and this stage runs what it is
// given, so a repository configuring a full lint over the whole tree gets that,
// slowly, and this stage does not notice.
//
	...
	return pipeline.Implementation{
		...
		NewBody: func() pipeline.Body {
			return func(ctx context.Context, in pipeline.Input) (pipeline.Output, error) {
				return runLintCheck(ctx, deps, in)
			}
		},
	}
}

// configuredLintCommand returns the lint check cfg names for this stage
...
// runLintCheck answers a run with it, and Holding predicts the hold with
// it - so the body and the prediction cannot drift apart.
func configuredLintCommand(cfg config.Config) (string, bool) {
	command := strings.TrimSpace(cfg.Commands.Lint)
	return command, command != ""
}

// runLintCheck is the stage body: it runs the configured check in the run's
// isolated copy and turns what the check answered into the stage's report.
func runLintCheck(ctx context.Context, deps StageDeps, in pipeline.Input) (pipeline.Output, error) {
	command, configured := configuredLintCommand(deps.Config)
	if !configured {
		return pipeline.Output{Report: noLintCommandConfigured()}, nil
	}

	repositoryID, runID, err := readLintState(in.State)
	if err != nil {
	...
	record := openLintEvidence(deps.Home.EvidenceLog(runID, in.Stage.String()), deps.redact)
	record.header(command, commit, copied.Path())
	result := runCommand(ctx, commandSpec{
		command:    command,
		dir:        copied.Path(),
		record:     record,
		projection: lintProjectionBytes,
		grace:      commandGrace,
		redact:     deps.redact,
	})
	...
	return pipeline.Output{Report: lintReport(deps.redact, command, commit, record, result)}, nil
}

// readLintState reads the two facts this stage needs of the run's state: which
...
// other.
func readLintState(state pipeline.Reader) (repositoryID, runID string, err error) {
...
// lintEvidence is the run's lint evidence file, and what went wrong if it
...
// openLintEvidence opens the run's lint evidence file for appending, creating the
...
func openLintEvidence(path string, r redact.Redactor) *lintEvidence {
	dir := filepath.Dir(path)
	if err := os.MkdirAll(dir, 0o700); err != nil {
		return &lintEvidence{path: path, redact: r,
			err: fmt.Errorf("making the evidence directory %s: %w", dir, err)}
	}
	file, err := os.OpenFile(path, os.O_CREATE|os.O_WRONLY|os.O_APPEND, 0o600)
	if err != nil {
		return &lintEvidence{path: path, redact: r, err: fmt.Errorf("opening %s: %w", path, err)}
	}
	return &lintEvidence{path: path, redact: r, file: file}
}
...
// output.
func (e *lintEvidence) Write(p []byte) (int, error) {
...
// to handle.
func (e *lintEvidence) mark(format string, args ...any) {
...
// printed.
func (e *lintEvidence) header(command, commit, dir string) {
	e.mark("=== lint stage\n=== command: %s\n=== commit:  %s\n=== copy:    %s\n"+
...
func (e *lintEvidence) footer(result commandResult) {
	switch {
	case result.exited:
		e.mark("\n=== exit status: %d\n\n", result.code)
	case result.err != nil:
...
func (e *lintEvidence) cutShort(err error) {
...
func (e *lintEvidence) recorded() bool { return e.err == nil }

// lintReport is what the stage found. The three cases are what a command can
...
func lintReport(r redact.Redactor, command, commit string, record *lintEvidence,
	...
	if record.recorded() {
		report.Evidence = []findings.Evidence{{
			Label: "this run's lint evidence: the output of the attempts at the configured lint " +
	...
	case !result.exited:
		report.Summary = fmt.Sprintf(
			"The configured lint command did not report an exit status against %s, so nothing was "+
		...
		report.Findings = append(report.Findings, findings.Finding{
			...
			Description: fmt.Sprintf(
				"The configured lint command did not report an exit status of its own, so it "+
					...
					"run.\n\ncommand: %s\ncommit: %s\n%s\n\n%s",
				command, commit, lintNotSettledReason(result), lintOutputSection(result, record)),
		})

	case result.code == 0:
		report.Summary = fmt.Sprintf(
			"The configured lint command passed against %s: %q exited zero in the run's isolated copy.",
			commit, command)

	default:
		report.Summary = fmt.Sprintf(
			"The configured lint command failed against %s: %q exited %d in the run's isolated copy.",
		...
		report.Findings = append(report.Findings, findings.Finding{
			...
			Description: fmt.Sprintf(
				"The configured lint command failed, so this change does not pass the static "+
					"analysis check this repository holds it to.\n\ncommand: %s\nexit status: %d\ncommit: %s\n\n%s",
				command, result.code, commit, lintOutputSection(result, record)),
		})
	}
	if !record.recorded() {
		report.Findings = append(report.Findings, findings.Finding{
			ID:       "lint-evidence-unrecorded",
			Severity: findings.SeverityWarning,
	...
	return redactLintReport(r, report)
}

// redactLintReport returns the report with every field this stage composes from
...
// command wrote.
func redactLintReport(r redact.Redactor, report findings.Report) findings.Report {
...
// lintNotSettledReason renders what os/exec reported about a command that never
// gave a status, and says so plainly when it reported nothing.
func lintNotSettledReason(result commandResult) string {
	if result.err != nil {
...
// lintOutputSection renders the command's output for a finding: the bounded tail,
...
// above.
func lintOutputSection(result commandResult, record *lintEvidence) string {
...
// noLintCommandConfigured is the report for a run whose configuration names no lint
// command.
//
// PRD section 10 makes commands.lint empty by default, so this is the ordinary
...
func noLintCommandConfigured() findings.Report {
	return findings.Report{
		Summary: "No lint command is configured, so this stage ran no check and established nothing " +
			"about this change.",
		Findings: []findings.Finding{{
			ID:       "lint-no-command",
			Severity: findings.SeverityWarning,
			Action:   findings.ActionAsk,
			Description: "No lint command is configured, so the lint stage had nothing to " +
				"run and could not gather evidence that this change passes static analysis. " +
				"Setting commands.lint in this machine's own configuration file is what gives this " +
