...
// hostFor is the forge.Host the stage is handed: it answers Open with the one
// modelled provider, and it refuses what the real GitHubHost.Open refuses -
// an empty specifier, before any request - with the same errors.Is identity,
// so no guard here rests on a shape the mechanism cannot produce.
type hostFor struct {
	provider forge.Provider
}

func (h hostFor) Open(repository string) (forge.Provider, error) {
	if repository == "" {
		return nil, fmt.Errorf("repository %w: it is required", forge.ErrInvalidArgument)
	}
	return h.provider, nil
}

var _ forge.Host = hostFor{}
...
func (r runState) state(t *testing.T) map[pipeline.Key]graph.Value {
	...
	out := map[pipeline.Key]graph.Value{
		...
		pipeline.KeyForgeRepository: graph.TextValue(r.forgeRepository),
...
// restriction on reads the stage node applies. A nil provider is a StageDeps
// nothing wired a code host into; anything else reaches the stage behind a
// hostFor, the way the real provider sits behind the service's forge.Host.
func runPR(t *testing.T, provider forge.Provider, run runState) (pipeline.Output, error) {
	t.Helper()
	var h forge.Host
	if provider != nil {
		h = hostFor{provider: provider}
	}
	impl := stages.PullRequest(stages.StageDeps{Forge: h})
...
// A run whose record names no repository on the code host reaches this stage
// with an empty pipeline.KeyForgeRepository, and forge.Host.Open refuses that
// rather than resolving a repository nobody chose. What the stage owes is
// returning that refusal as its own failure, before anything reaches the
// provider.
//
// The positive control is the same body over the same host with the
// repository named: without it this would pass just as well if the stage
// failed on everything.
func TestThePullRequestStageFailsWhenTheRunNamesNoRepositoryOnTheCodeHost(t *testing.T) {
	t.Parallel()
	h := newHost()
	run := aRun()
	run.forgeRepository = ""
	out, err := runPR(t, h, run)
	if err == nil {
		t.Fatalf("the stage reported %+v for a run naming no repository on the code host, "+
			"rather than failing", out.Report)
	}
	if !errors.Is(err, forge.ErrInvalidArgument) {
		t.Fatalf("the failure %q is not the host's refusal of the empty specifier", err)
	}
	if len(h.calls) != 0 {
		t.Fatalf("the stage reached the provider anyway: %v", h.calls)
	}

	if _, err := runPR(t, h, aRun()); err != nil {
		t.Fatalf("the stage failed with the repository named too, so the assertion above "+
			"does not distinguish a missing repository from anything else: %v", err)
	}
}
...
func TestThePullRequestStageDeclaresEveryKeyItReads(t *testing.T) {
	...
	impl := stages.PullRequest(stages.StageDeps{Forge: hostFor{provider: newHost()}})
