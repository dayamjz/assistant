...
// stage a run here stops at holds on a finding with the action ask - a
// command nobody configured, a review nobody completed, a code host nobody
// named - so a run holds at every one of them for a person rather than
// reporting a pass it did not establish, and no such finding enters a fix
// round. Which stages those are is this harness's declared stops rather than
// a count to nine, because a body that lands moves where a run stops. That is
// driven over the whole walk rather than over the first hold: a claim about
// every such stage that rested on one would be a count nothing checked.
func TestAFindingThatIsNotClassifiedStopsForAPerson(t *testing.T) {
	...
	t.Run("through the binary", func(t *testing.T) {
		...
		// reasons walkableRun states.
		walk := answerHolds(t, j,
			walkableRun(t, j, "a change whose run stops at every declared hold"), "approved")
...
type stopped struct {
	...
	holds []machine.Run
	// stages is how many stages this harness declares a run stops at, which
	// is how many holds a run has to reach: every other stage reports what it
	// established and goes on.
	stages int
}
