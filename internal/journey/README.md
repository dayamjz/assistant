...
| Outside linux and darwin there is also no local socket transport for the service to serve this protocol over, so a service does not come up there at all. `internal/service` and `internal/cli` both carry a skip for exactly that, bounded by the same written-down platform predicate. | Skips every check that serves, rather than failing for the platform. One helper starts every service these tests start, and it is the one call to `journey.Serve` here so that the guard cannot be forgotten; nothing enforces that, and the helper's own comment says why and names it a rule rather than a mechanism. The skip takes the checks above with it and two more that serve without driving a run, P1's ordinary push and P8's authoritative state, whose `Coverage` notes say so; on such a platform no check here that needs a service runs. A failure to serve anywhere the transport does exist stays a failure. |
