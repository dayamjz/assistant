...
| Outside linux and darwin there is also no local socket transport for the service to serve this protocol over, so a service does not come up there at all. `internal/service` and `internal/cli` both carry a skip for exactly that, bounded by the same written-down platform predicate. | Skips every check that serves, through the one helper all of them go through, rather than failing for the platform. That takes the checks above with it and two more that serve without driving a run, so on such a platform what a green run establishes is narrower again: no check here that needs a service runs. A failure to serve anywhere the transport does exist stays a failure. |
