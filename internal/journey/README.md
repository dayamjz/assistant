...
input P2 gives a person. The test stage's holds for the command nobody
configured here rather than executing anything. Every other stage has no body
and reports one unclassified finding and holds for a person, which is P3
working as specified. Between them, that is the reason large parts of the
product are not reachable from a run today:
