...
| Outside linux and darwin there is also no local socket transport for the service to serve this protocol over, so a service does not come up there at all. `internal/service` and `internal/cli` both carry a skip for exactly that, bounded by the same written-down platform predicate. | Skips every check that needs a service, rather than failing for the platform, so a green run there establishes exactly the checks that need none. Which those are is not listed here, for the reason the row above gives: a list of them rots the next time a check is added, and this file does not own it. The `Coverage` note of every principle the skip takes down says so, and a `-v` run names each check that skipped. Two helpers start every service these tests start and both take the guard before starting one; nothing enforces that no test starts one another way, and their own comment says so and names it a rule rather than a mechanism. A failure to serve anywhere the transport does exist stays a failure. |
