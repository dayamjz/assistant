	...
	return []Drove{
		...
		{"refusal-hostile-harness-installation", ReachNone, "",
			"Neither half of what this condition records is established, at any reach. Its Value is the " +
				...
				"launches nothing, the review body fails on the isolated copy nothing creates " +
				"before it launches, and the pull request body fails because the run's record " +
				"names no repository on the code host, so a run skips both of those stages - and a run " +
