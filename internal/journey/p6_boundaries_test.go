...
func TestARunSurvivesTheServiceBeingKilledAtEveryStageBoundary(t *testing.T) {
	...
	// A run stops at every stage this harness declares it does: every stage
	// with no body, and the push stage, whose body holds every run this build
	// can produce. The declaration rather than a count to nine is what moves
	// when a body lands, so a check written against the stage count would fail
	...
	if current.Outcome == machine.OutcomeDecision {
		t.Fatalf("the run was killed and answered at %d boundaries and is still holding at %s; this "+
			"harness declares %d stage(s) a run stops at", len(observed.boundaries), current.Position, len(holding))
	}
	...
	// derived from. How many boundaries a run reaches is how many stages the
	// declaration says a run stops at, so a literal position stops being in
	// range the day a body lands and moves one, and a counterfeit that
	// panicked would take the check down with something that is not a finding.
				...
				Holds: func(s survival) error {
					if len(s.boundaries) != len(holding) {
						return fmt.Errorf("the service was killed at %d boundaries and this harness "+
							"declares %d stage(s) a run stops at, each of which is a boundary",
